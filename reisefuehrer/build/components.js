...
function card(inner, { tag = '', tagVariant = '' } = {}) {
...
function dishCard(d, ICONS) {
  ...
  return card(`
    ...
    ${d.tip ? `<p class="tiny"><b>Tipp:</b> ${esc(d.tip)}</p>` : ''}
  `);
}
...
function restaurantCard(r) {
  ...
  return card(`
    ...
    ${r.pick ? `<p class="tiny"><b>Bestellen:</b> ${esc(r.pick)}</p>` : ''}
  `, { tag: r.category, tagVariant: 'coral' });
}

function medalCard(rank, a) {
  // rank: 1|2|3, a: {emoji, name, desc, price, duration, link, why}
  const cls = rank === 1 ? 'gold-rank' : rank === 2 ? 'silver-rank' : 'bronze-rank';
  const medal = rank === 1 ? '🥇' : rank === 2 ? '🥈' : '🥉';
  return `<div class="medal-card ${cls}">
    ...
    ${a.why ? `<p class="tiny"><b>Warum:</b> ${a.why}</p>` : ''}
    ${a.link ? `<p class="tiny">🔗 ${esc(a.link)}</p>` : ''}
  </div>`;
}

function medalRow(activities) {
  return `<div class="medal-row">${activities.map((a, i) => medalCard(i + 1, a)).join('')}</div>`;
}
